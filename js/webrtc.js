...
var PHONE = window.PHONE = function(config) {
    ...
    var mediaconf     = config.media || { audio : true, video : false };
    var conversations = {};
    var oneway        = false
    var broadcast     = false;
   ...
   // -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    var rtcconfig = { iceServers :  [{"url":"turn:35.161.239.237","credential":"concrema","username":"flan"}]};
...
//	] };
    ...
    function get_conversation(number) {
        var talk = conversations[number] || (function(number){
 	    ...
 	    if (mystream) talk.pc.addStream(mystream);   // Add null here on the receiving end of streaming to go one-way.
